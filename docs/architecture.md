...
- `backend/src/rosita/core/ai_client.py`: clientes de IA (Ollama, OpenRouter, Gateway OpenAI-compatible).
...
O agente abstrai tres provedores via `create_ai_client()`:

| Provedor | Classe | Configuracao (.env) |
|----------|--------|---------------------|
| Ollama | `OllamaClient` | `ROSITA_OLLAMA_HOST`, `ROSITA_OLLAMA_MODEL` |
...
Provedor inicial: `ROSITA_AI_PROVIDER` (`ollama`, `openrouter` ou `gateway`). O administrador pode alternar provedores em runtime quando mais de um estiver configurado.
...
4. O cliente do provedor ativo (Ollama, OpenRouter ou Gateway) responde em streaming.
